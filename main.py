...
Capabilities (each toggled independently in config):
  - Teams: read channel messages, send channel messages via Power Automate Workflow
...
Slash commands:
  ...
  /m365-teams-send <msg>         — post a message via Power Automate Workflow
...
Note: Teams channel posting uses a Power Automate / Logic Apps Workflow
(HTTP trigger → Post message to channel). Configure teams_workflow_url in
plugin settings. See README.md for setup instructions.
...
class M365Plugin(Star):
    def __init__(self, context: Context, config: dict = None):
        ...
        self.teams_workflow_url = str(self.cfg.get("teams_workflow_url", "") or "").strip()
    ...
    async def _send_teams_message(self, text: str) -> str:
        """Post a message via Power Automate Workflow (Logic Apps HTTP trigger)."""
        if not self.teams_workflow_url:
            return (
                "⚠️ teams_workflow_url is not configured.\n"
                "Create a Logic Apps Consumption workflow with an HTTP trigger → "
                "Teams 'Post message in a channel' action, then paste the trigger URL in plugin config."
            )
        ...
        async with self._graph._session.post(
            ...
            if not resp.ok:
                ...
                raise RuntimeError(f"Workflow POST failed {resp.status}: {body}")
    ...
    async def cmd_teams_send(self, event: AstrMessageEvent):
        ...
        Post a message to the Teams channel via Power Automate Workflow.
